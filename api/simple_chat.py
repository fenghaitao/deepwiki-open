...
from api.config import get_model_config, configs, OPENROUTER_API_KEY, OPENAI_API_KEY, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, GITHUB_TOKEN
...
class ChatCompletionRequest(BaseModel):
    ...
    # model parameters
    provider: str = Field("google", description="Model provider (google, openai, openrouter, ollama, bedrock, azure, github)")
...
async def chat_completions_stream(request: ChatCompletionRequest):
    ...
    try:
        ...
        if request.provider == "ollama":
            ...
            # GitHub Copilot uses OAuth2 authentication, GITHUB_TOKEN is optional
            if not GITHUB_TOKEN:
                logger.info("GITHUB_TOKEN not set - GitHub Copilot will use OAuth2 authentication")
            else:
                logger.info("Using GITHUB_TOKEN for GitHub Copilot authentication")
